    ...
    for i in range(len(layers)):
        ...
        if args.nearest:
            ...
            for name in subset:
                ...
                # W = subset[name].weight.data
                # W,L,R = get_best_sketch_svd(W, fix_rank = 32)
                # quantizer.find_params(W, weight=True)                
                # subset[name].weight.data = quantize_lora(
                #     W, quantizer.scale, quantizer.zero, quantizer.maxq, L, R
                # ).to(next(iter(layer.parameters())).dtype)

                W = subset[name].weight.data
                max_abs_value0 = torch.max(torch.abs(W))
                W,L,R,m0,mnow,workrank = get_best_sketch(W,4, ratio = 0.4, fix_rank = 0)
                max_abs_valueNOW = torch.max(torch.abs(W))
                quantizer.find_params_mini2(W, weight=True) 
                #quantizer.find_params_mini(W, weight=True)     
                print(f"{name}, rank:{workrank}, {W.shape}, m0:{max_abs_value0}, mnow:{max_abs_valueNOW}, {quantizer.scale.shape}, {quantizer.zero.shape}, {next(iter(layer.parameters())).dtype}")           
                subset[name].weight.data = quantize_lora_group(
                    W, quantizer.scale, quantizer.zero, quantizer.maxq, L, R,workrank
                ).to(next(iter(layer.parameters())).dtype)
                #print(f"{name}, {max_abs_value0},{torch.max(torch.abs(subset[name].weight.data))}")
